...
for max_depth in [None, 5, 10, 15, 20]:
    random_forest_pipeline = Pipeline(steps=[
        ...
        ("model", RandomForestRegressor(n_estimators=num_est, max_depth=max_depth))
    ])

    # Run cross validation to get a reliable performance estimate across 5 different train/validation splits
    scores = cross_val_score(random_forest_pipeline, X,
                             y, cv=5, scoring="neg_root_mean_squared_error")

    print(
        f"Random Forest max_depth = {max_depth}, n_estimators = {num_est}: avg RMSE = {-scores.mean():.0f}, fold range = {-scores.max():.0f} - {-scores.min():.0f}")
